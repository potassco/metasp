...
The instances and encodings for the benchmarks can be found in the `instances`
directory.

The benchmarks were run using the
[Potassco Benchmark Tool](https://potassco.org/benchmark-tool/). To run the
benchmarks for both the TEL and DEL domains, one must run the following
scripts.
...
- Metasp ran using
  [metasp v1.0.0](https://github.com/potassco/metasp/releases/tag/v1.0.0)
- Telingo ran using
  [telingo-lambda v2.1.3](https://github.com/krr-up/telingo-lambda/releases/tag/v2.1.3)
...
Results for the elevator domain (DEL) are shown below. Instances are named as
`fX-nY`, where `X` is the number of floors and `Y` is the horizon. Values are
shown as **Without Control / With Control**.
