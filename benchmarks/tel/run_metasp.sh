...
CSV="$SCRIPT_DIR/metasp_results_v2.csv"
...
for domain in "${DOMAINS[@]}"; do
    ...
    for instance in "$SCRIPT_DIR/instances/${domain}"/*.asp; do
        instance_name="$(basename "${instance%.asp}")"
        ...
        echo "Running: metasp solve clingo 1 -q --meta-config $SCRIPT_DIR/config.yml --stats -c n=$n $instance $encoding"
        output=$(timeout "$TIMEOUT" metasp solve clingo 1 -q \
