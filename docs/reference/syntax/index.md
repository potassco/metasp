...
> **Note:** The `clingo_const_term` and `identifier` productions are
> the same as in the standard clingo language; constant term being a
> term that contains no variables.ð

## Example metasp type declaration

Below we provide a type declaration for a fragment of Temporal Equilibrium Logic.
